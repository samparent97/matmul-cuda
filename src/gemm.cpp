  ...
  void gemmEfficientParallel(int m, int n, int k, const float *A, const float *B, float *C, ScheduleParams Sp) {
    ...
    if( m >= k * 100){
        ...
        int jLim;
            ...
            // Since m >> k, parallelize over m dimension and keep k,n blocks together
            // Reorder loops to maximize cache reuse of B matrix

            #pragma omp parallel default(none) private(i, j, l, ii, ll, jj, iLim, kLim, jLim) shared(A, B, C, m, n, k, BLOCK_SIZE)
            {
            for (ii = omp_get_thread_num() * BLOCK_SIZE; ii < m; ii += BLOCK_SIZE * omp_get_num_threads()) {
        ...
        for (int i = 0; i < m; i++) {
            memcpy((void*)(Apad + i * kAdj), (void*)(A + i * k),
                  k * sizeof(float));
        }


        for (int i = 0; i < k; i++) {
            memcpy((void*)(Bpad + i * nAdj), (void*)(B + i * n),
                  n * sizeof(float));
        }
                ...
                for (int cc = 0; cc < numCColBlocks; ++cc) {
                    ...
                    for (int ac = 0, br = 0; ac < numABBlocks; ++ac, ++br) {
                        ...
                        for (int r = 0; r < s1; ++r) {
                            for (int c = 0; c < s1; c += 8) {
                                auto load_vec = _mm256_loadu_ps(
                                    &Bpad[(((br * s1) + r) * nAdj) + (cc * s1 + c)]);
                    ...
                    for (int r = 0; r < s1; ++r) {
                        for (int c = 0; c < s1; c += 8) {
                            ...
                            _mm256_storeu_ps(&Cpad[(((cr * s1) + r) * nAdj) + (cc * s1 + c)],
                                            store_vec);
                        }
                    }
                }
        ...
        for (int i = 0; i < m; ++i) {
            memcpy((void*)(C + i * n), (void*)(Cpad + i * nAdj),
                  n * sizeof(float));
        }
